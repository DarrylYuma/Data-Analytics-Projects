# Dashboard
<img width="1381" alt="Dashboard Image" src="https://github.com/DarrylYuma/Excel-Data-Analysis-Projects/blob/main/Adidas%20US%20Sales%20Analysis/Adidas%20US%20Sales%20Dashboard.png">

# Adidas US Sales Analysis
This project is an analysis and dashboard visualization built using a mock dataset of sales in the US by the brand Adidas. The dataset is first cleaned and then a dashboard is created that produced the following visualizations:
- Sales Trend Over Time: A line graph that shows sales performance over time, allowing comparison of sales between different products and different regions.
- Revenue per Retailer: A combo chart that showcases the total sales and operating profit for each retailer, allowing for easy visualization of which retailer is performing and contributing the most revenue.
- Sales per Region: A map chart that showcases regional performance of sales.
- Sales per Sales Method: A pie chart that showcases sales performance based on different sales methods, highlighting the significance of certain sales channels to sales performance.

The created dashboard can be interacted with to provide further insight into specific products, regions, and year time frames with the use of slicers. In summary, the dashboard created in this project allows the user to analyze and dissect data by region or product to identify trends that may lead to key business insights.
# Source
[Kaggle](https://www.kaggle.com/datasets/heemalichaudhari/adidas-sales-dataset/data)
